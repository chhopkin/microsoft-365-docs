...
|Microsoft 365 Copilot with computer-using agent|Microsoft 365 Copilot with computer-using agent allows customers to automate browser or desktop tasks. It's integrated into Copilot so that your prompts can leverage the power of all your content and context. The agent has multiple safeguards, including model-driven safety checks over content and actions as well as tenant admin controls that enable or disable the computer-using agent and restrict which internet sites and domains the agent can access.|
