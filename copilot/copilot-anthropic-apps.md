...
> [!NOTE]
> Support for Anthropic models in Microsoft 365 apps varies based on how the AI providers operating as Microsoft subprocessors setting is scoped in your organization. When the setting is enabled for **All users**, Anthropic model use in supported apps is unavailable and can't be changed. When the setting is scoped to **No users**, Enterprise, public sector, and EDU tenants have Anthropic model use in apps turned off by default but can enable it, while non‑Enterprise tenants have Anthropic model use turned on by default but can disable it. This allows admins to control whether Anthropic model processing can be flexibly routed within the EU. This setting applies only to Copilot experiences within supported apps and doesn't affect Anthropic model usage in other Microsoft 365 Copilot features or services. For more information, see [Flex routing (EU and EFTA)](/copilot/microsoft-365/copilot-flex-routing).
