---
title: "Management controls in Copilot Control System"
description: "Management controls in the Copilot Control System framework enable you to manage and deploy your Microsoft 365 Copilot licenses and agents."
author: aczechowski
ms.author: aaroncz
manager: dansimp
ms.reviewer: bensum
ms.service: microsoft-365-copilot
ms.topic: solution-overview
ms.date: 08/06/2025
---

# Management controls in Copilot Control System

Copilot Control System management controls enable you to determine how your Microsoft 365 Copilot licenses and agents are deployed to meet the unique needs of your organization. Copilot and agent management controls are found primarily but not exclusively in the [Microsoft 365 admin center](/microsoft-365/admin/admin-overview/admin-center-overview), [Power Platform admin center](/power-platform/admin/), and [Copilot Studio](/microsoft-copilot-studio/fundamentals-what-is-copilot-studio).

:::image type="content" source="media/management-controls.png" alt-text="A diagram of the Copilot Control System framework highlighting the Management Controls pillar.":::

> [!NOTE]
> Three main pillars make up the [Copilot Control System](overview.md):
>
> - [Security and governance](security-governance.md)
> - **Management controls** (this article)
> - [Measurement and reporting](measurement-reporting.md)

The management controls pillar of the Copilot Control System focuses on the following key capabilities:

- Licensing and metering
- Agent lifecycle
- Customization

## Licensing and metering

You can manage Microsoft 365 Copilot license assignment and control access to agents ensuring compliance with policies, usage limits, and more.

## Agent lifecycle

The system provides visibility into the status, governance, and lifecycle of agents and allows administrators to review agent details, approve, block and manage usage. It also enables administrators to control environmental routing, groups, and rules that support organizational policies.

## Customization

Administrators can configure pay-as-you-go billing, review agent message consumption, and manage agent consumption costs.

## Related content

- [Manage Microsoft 365 Copilot scenarios](/copilot/microsoft-365/microsoft-365-copilot-page)

- [Manage agents for Microsoft 365 Copilot](/copilot/microsoft-365/microsoft-365-copilot-page)

- [Copilot Control System - Microsoft Adoption](https://adoption.microsoft.com/copilot/control-system/)
