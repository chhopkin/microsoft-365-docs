...
The Coworker agent in Microsoft 365 Copilot carries out tasks on your behalf&mdash;sending emails, creating documents, scheduling meetings, and searching across your organization. This guide covers everything you can do with Coworker.

## Start a conversation

The Coworker home page has a chat input where you can type or speak your request. Below it, your recent tasks are listed so you can resume a previous conversation.
...
1. Press **Enter** or select the send button to submit.

### Attach files

You can include files with your message to give Coworker extra context.

1. Drag and drop files directly onto the chat input area, or select the file picker button to browse your device.
1. Wait for the upload to finish. You'll see an animated progress chip for each file.
1. Add your message text if needed, then send.
...
1. Select the agent you want. It appears as a pill in the input field so you know who you're talking to.
...
1. Edit the text if needed, then send.

> [!TIP]
> Check the home page for your recent tasks. Select any task to jump straight back into that conversation without starting over.
...
As Coworker works on your request, a few things happen on screen:

- **Thinking indicator** — An animation lets you know Coworker is processing your message.
- **Step-by-step updates** — You see what Coworker is doing at each stage, such as "Composing your email" or "Searching OneDrive."
- **Streaming response** — Coworker's reply appears word by word as it's generated, so you don't have to wait for the entire response.
...
If you think of something else while Coworker is still working, you can send another message. It is queued automatically — you see it as a pending pill above the input field. Coworker processes queued messages in order.

### If something goes wrong

If a message fails to send, Coworker shows you an error with a **Retry** option. Select **Retry** to resend the message.
...
1. When an approval is needed, a dialog appears with the details of the action Coworker wants to take. For some actions, you'll see a rich preview of the content (for example, a draft email or a Teams message). For other actions, you'll see a summary of what Coworker plans to do.
1. Choose one of the following options:

   | Option | What it does |
   |---|---|
   | **Approve** | Allows Coworker to proceed with the action this one time. |
   | **Approve and remember** | Allows Coworker to proceed and remembers your choice for similar actions in the future, so you won't be asked again. |
...
1. Find the file under the **Output** group in the **Files** section.
1. Select **Download** to save it to your device.
...
You can preview many file types right inside Coworker — no need to download them first. The preview opens alongside your conversation in a split view.

### Supported preview formats

The following file types can be previewed directly:

- **PDF** — Rendered inline with page navigation.
- **CSV** — Displayed as a formatted table.
- **Markdown** — Rendered with full formatting.
- **Images** — Displayed inline (png, jpg, jpeg, gif, webp, bmp, svg, ico).
- **HTML** — Rendered in the preview pane.

### Preview a document

1. Select a file from the conversation or from the **Files** section in the side panel.
1. The preview opens in a split view next to your chat.
...
| **Skills** | Skills that Coworker has used during the conversation, shown as chips. |
...
- **Pause (hard)** — Coworker pauses immediately, even mid-step. Use this when you need to stop now.
...
Select **Cancel** to stop Coworker's current work entirely. Canceling clears the current task so you can send a new message without needing to resume.
...
You can browse your OneDrive files and folders directly from Coworker to pull in the files you need.

1. Open the OneDrive file browser from the file selection interface.
...
1. Select a file to include it in your conversation.

> [!NOTE]
> If your files don't load, Coworker shows a **Retry** button. Select it to try again.

## Coworker skills

Coworker comes with a set of built-in skills — specialized abilities that let it handle different types of tasks. When you ask Coworker to do something, it automatically picks the right skill for the job.
...
You can see which skills Coworker has used during a conversation in the **Skills** section of the side panel, where they appear as chips.

## Provide feedback

Your feedback helps improve Coworker. There are several ways to share it.
...
1. A dialog opens showing all available shortcuts and their key combinations.

> [!TIP]
> Take a moment to explore the keyboard shortcuts — they can save you a lot of time once you learn the ones you use most.
