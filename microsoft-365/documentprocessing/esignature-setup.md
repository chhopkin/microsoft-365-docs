...
If the Word or PDF documents to be signed reside in a site with a sensitivity label, the label may prevent users from sending requests to external users. To remedy, enable the label setting **Let people who use this label share the site with people outside your organization** or move the documents to a site that has a label with that setting enabled. [Use sensitivty labels with SharePoint sites](/microsoft.com/en-us/purview/sensitivity-labels-teams-groups-sites)
