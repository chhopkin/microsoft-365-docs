...
|Reactivating     |If a site or file is reactivated from the “*Archived*” state, it remains in this stage until reactivation completes. After reactivation, the content becomes active. When a site finishes reactivation, it appears on the Active sites page. Files that are reactivating remain in the same location after reactivation completes. | None for sites. Delete for files.|None for sites.  Delete for files.       |
