...
An admin can choose not to deploy an app that is linked to a SaaS offer. On returning to the integrated apps portal, admins will find the status of **More apps available** in the Deployed Apps list view. This status is to inform the admins that there are more integrations from the ISVs that haven’t been deployed yet.

## Unsupported scenarios

There are some scenarios when SaaS linked apps can't be managed from the integrated apps portal. You are not able to deploy a single store add-in from the integrated apps portal for the following scenarios:
