...
Currently, the following types of extensions cannot be included in the package of an app for Microsoft 365, and are not considered apps for Microsoft 365.

- OneDrive and SharePoint extensions. But note that these types of extensions can be combined with an app for Microsoft 365 in a [SAAS linked bundle](saas-linked-app.md). Also, note that some apps for Microsoft 365, such as personal tabs can be implemented as SharePoint Framework pages.
...
There is a type of extension that can surface in multiple host applications even though it is just a single app, not multiple apps configured in the same manifest. For guidance in managing them, see the following. 
