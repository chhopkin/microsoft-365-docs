...
The Agent 365 Overview page in Microsoft 365 Admin Center serves as the control plane for all agents. It provides IT administrators with a centralized dashboard to:
...
You can view Overview by going to Agents > Overview, or by using AI home page > Click on View in Agent 365 overview.

## Key Features

- Provide a snapshot of agent health and actionable insights for the last 30 days.
- Highlight critical actions for admins (e.g., approve pending requests, manage agents with alerts, exceptions, etc.).
...
- Agent Inventory: The total count of all agents available in your organization's catalogue, including Microsoft-built, partner-built, and custom (LOB) agents. This reflects the breadth of automation capabilities deployed across your tenant. You can directly click on the Explore All agents > Inventory to go through your inventory in detail.
...
Definition: The estimated cumulative hours saved by employees through agent-assisted tasks, based on modeled productivity gains.. Demonstrates ROI and business impact.
...
- Agents by Publishers: Breakdown of all agents in your inventory by their source of publisher (who created and shared it). There are two categories in this:
  ...
  - It can be Created by your organization: shared by creator, used only by creator, published by your org etc.
  - It can be Created by external partners.

- Agents by Platforms: Shows which creation platforms (e.g., Copilot Studio (Full/Lite), Azure AI Foundry, any external partner platforms etc.) are most used for building agents.
...
Manage requests button: To take immediate action you can directly click on "Manage requests" button on the card.  Navigates to the Agent Inventory → Requests tab. This tab lists all pending agent requests submitted by users within organization.

- Ownerless Agents: Total Agents without an assigned owner, still pending owner assignment.
Assign owner button: You can click directly on the "Assign Owner" button. It takes you to the Agent Inventory → Ownerless Agents filter, showing agents without an assigned owner.
