...
Use idle session timeout to configure a policy on how long users are inactive in your organization before they're signed out of Microsoft 365 web apps. Automatically signing out inactive users helps protect sensitive company data and adds another layer of security for end users who work on non-company or shared devices.
...
1. On the **Idle Session Timeout**, select the toggle to turn it on. You can choose a default setting or choose your own custom time. It takes a few minutes before idle session is turned on in your organization.
...
When a user is inactive in Microsoft 365 web apps for the time period you chose, they see a **Your session is about to expire** window. They have to select the **Stay signed in** button in the window or they're signed out.
...
1. Select **Users or workload identities**, and then select **All users**.

1. Select **Cloud apps or actions**, **Select apps**, and search for **Office 365**. Select **Office 365**, and then **Select**.

1. Select **Conditions**, **Client apps**, **Configure to Yes**, **Browser**, and then select **Done**.

1. Select **Session**, **Use app enforced restrictions**, and then **Select**.

1. Turn on the policy and select **Create**.
...
1. In the dropdown menu, select a different timeout value and then **Save**.

Delete the policy:

1. In the Microsoft 365 admin center, select **Settings** > **Org settings**, go to the **Security & Privacy** tab and select **Idle session timeout**.

1. Uncheck **Turn on to set the period of inactivity for users to be signed off of Microsoft 365 web apps** and select **Save**.
