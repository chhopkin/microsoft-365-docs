...
The Group Policy setting **Block OrgChart** can be found in the following path and registry key:

- **Group Policy path:** `User Configuration > Administrative Templates > Microsoft Office 2016 > Security Settings > Block OrgChart`
- **Registry key path**: `HKCU\Software\Policies\Microsoft\Office\16.0\Common\Security > BlockOrgChart`

To revert to the default behavior, set this policy as not configured.

The Group Policy setting **Block OLE Graph** can be found in the following path and registry key:

- **Group Policy path**:  `User Configuration > Administrative Templates > Microsoft Office 2016 > Security Settings > Block OLE Graph`
- **Registry key path**: `HKCU\Software\Policies\Microsoft\Office\16.0\Common\Security > BlockOleGraph`
