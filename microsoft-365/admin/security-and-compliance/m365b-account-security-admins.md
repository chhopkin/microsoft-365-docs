...
Although multifactor authentication (MFA) is [turned on by default for all accounts in Microsoft 365 for business organizations](m365b-account-security.md), administrator or admin accounts require extra security considerations unrelated to MFA. Admin accounts have elevated privileges and are valuable targets for attackers. At the same time, admin accounts are fundamental to the operation and management of your organization.
...
- **Be sure to have more than one admin account**: We recommend at least two admin [emergency access accounts](/entra/identity/role-based-access-control/security-emergency-access) in every organization that aren't assigned to specific individuals and are only used in emergencies. These accounts also need to be [excluded from MFA requirements](m365b-account-security.md#step-3-adjust-mfa-exclusions) and therefore should have long, complex passwords (16 or more characters). These accounts are also known as "break glass accounts."

  For admin account creation instructions, see [Add an admin](/microsoft-365/admin/add-users/assign-admin-roles#steps-add-an-admin).
...
- **Use ordinary user accounts for regular work tasks**: Admins should use non-admin accounts as primary accounts to sign in to their computers and do regular tasks like checking email and using Microsoft 365 Apps. Use your admin account only as needed.
  ...
  For account creation instructions, see [Add users and assign licenses at the same time](/microsoft-365/admin/add-users/add-users).
...
- **Use passwordless authentication for admin accounts**: For more information, see [Enable passwordless sign-in with Authenticator](/entra/identity/authentication/howto-authentication-passwordless-phone).
  ...
  - Microsoft 365 Business Premium via Microsoft Entra ID Plan 1 also allows you to _require_ passwordless authentication and authentication strength for admin accounts using Conditional Access. For more information, see [Conditional Access authentication strength](/entra/identity/authentication/concept-authentication-strengths).
