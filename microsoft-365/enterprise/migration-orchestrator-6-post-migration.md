...
- Use the `Remove-CtimServicePrincipal` cmdlet to remove permissions for identity mapping (both tenants).

- Use the `Revoke-CTTMAppPermissions` cmdlet to remove permissions for Teams Chat migrations (both tenants).

- Use the following to remove CTMS (Cross-Tenant Migration Service) permissions and service principals. This action is only required on the target tenant:
