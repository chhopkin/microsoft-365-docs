...
It's important to be in the correct MsGraph context when running commands from the source and target tenants. To confirm you're correctly connect, use Disconnect-MgGraph, Connect-MgGraph, and Get-MgContext.
...
The following table lists the checks performed in the Pre-Migration validation stage of the migration. After the submission of the migration batch, these checks are the first tasks processed. If any of them hit an error, then the migration is cancelled so any errors can be remediated. After fixing the errors, the admin can submit a new batch with the same users.

|Check |Error message |Troubleshooting |
|------|--------------|----------------|
|A user can't belong to a preexisting batch |User migration validation failed. User is already a part of same batch. </br>User migration validation failed. User is already a part of another batch {0} |If a user belongs to multiple batches, the user's only migrated once. Likewise, if the user's listed twice within the same batch, it's migrated once. </br>Admins must resubmit the migration batch without the duplicate user. |
...
|Each source user needs to have been mapped to a target user (Identity Mapping) |Teams migration for user {0} failed. User {1} from source tenant {2} is not mapped to an identity in target tenant {3}. Refer to documentation for Identity Mapping and retry migration. </br>Teams migration for user {0} failed. There is a conflict in the target user provided by the migration service and the Identity Mapping service. Rerun Identity Mapping and retry migration. |Add the users to identity mapping. |
