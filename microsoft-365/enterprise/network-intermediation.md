...
### Why You Should Not decrypt or manipulate Microsoft 365 Traffic at the network layer

:::image type="content" source="media/network-intermediation/tls-intermediation.png" alt-text="Diagram showing network decryption being performed for Microsoft 365 traffic by a network intermediation device. This isn't a supported scenario.":::
...
#### Network Insights in admin center: Detecting Problematic patterns like decryption of Microsoft 365 traffic at the network layer
...
- **"Decryption/Encryption of Microsoft 365 traffic detected"** - Microsoft can often detect when large amounts of your tenant's traffic appear to be proxied or tampered with. If 30% or more of Microsoft 365 requests from a location show signs of network intermediation, an insight alert is generated. It essentially warns: "User connections to Microsoft 365 domains are being decrypted by a network intermediary" This alert is usually accompanied by guidance to stop doing that (i.e. bypass the network service responsible for decryption for these endpoints). It may even identify the likely network service vendor involved (when possible) so you know where to look, the Insight also lists the impacted Microsoft 365 domains in "Affected endpoints" section to you know what domains to bypass.

- **"Copilot impacted by WebSocket failures"** - As mentioned, if the service sees that WebSocket connections are failing, the portal can flag that with the Insight "Critical domain WebSocket connectivity failures detected in your tenant" for apps like Copilot. This shows up as an insight tying network issues to specific feature impact (e.g., *Copilot isn't working due to network interception*).

These insights appear on the [Microsoft 365 connectivity dashboard](https://admin.cloud.microsoft/#/networkperformance) in the Admin Center and are also delivered via Service Health notifications. They give IT admins a heads-up that something in the network is interfering with Microsoft 365 - and often point to inspection at the network layer. This proactive visibility is extremely helpful; it takes the guesswork out of diagnosing issues which can otherwise take days or weeks to troubleshoot. 

*(See reference: [Microsoft 365 Network Insights - Microsoft 365 Enterprise](/microsoft-365/enterprise/office-365-network-mac-perf-insights))*
...
- *Treat Microsoft 365 traffic as trusted.* Use Microsoft's published endpoints to identify it, and allow it to bypass your generic internet filters. This doesn't mean no security - it means relying on Microsoft 365's security (which is very robust) instead of yours for that traffic.

- *Egress locally, connect directly.* Aim for every office (and even home users) to connect to Microsoft 365 without hairpins. This often means provisioning local internet breakout and DNS resolution. The result is users connect to Microsoft's network at a nearby point, often resulting in 50%+ latency reduction versus tromboning through a distant HQ.

- *Don't break what you're trying to protect.* Intercepting Microsoft 365 traffic is well-intentioned (you want to scan for threats), but Microsoft 365 already has those protections. By intervening, you actually *reduce* security (end-to-end encryption is weakened) and performance. Instead, allocate those security devices to inspect unknown web traffic or third-party services where they're truly needed.

- *Use Microsoft's monitoring.* Leverage tools like the [Microsoft 365 connectivity dashboard](https://admin.cloud.microsoft/#/networkperformance) in the Admin Center. Microsoft can often tell you if your network setup is suboptimal for Microsoft 365. If you see an insight about TLS decryption - act on it. Microsoft wouldn't flag it if it wasn't important.
